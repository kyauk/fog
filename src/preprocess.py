...
class DataPreprocessor():
    ...
    def create_windows(self,data):
        ...
        windows = []
        window_iter = int(self.window_size * self.sampling_rate)
        overlap_iter = int(self.window_overlap * self.sampling_rate)
        for sample in range(0, len(data) - window_iter + 1, overlap_iter):
                end = sample + window_iter
                windows.append(data[sample:end])
        return np.arrays(windows)
